...
const Recipe = (props) => {
  ...
  const handleCheckboxClick = (item) => () => {
    setNeededIngredients({
      ...neededIngredients,
      [item]: !neededIngredients[item],
    });
  };
  const handleSubmit = (event) => {
    event.preventDefault();
    const ingredientsToAdd = Object.keys(neededIngredients).filter(
      (ingredient) => neededIngredients[ingredient]
    );
    ...
    <div>
      ...
      >
        ...
        <Card.Body className="card-body">
          <Card.Title className="cardTITLE">{label}</Card.Title>
        </Card.Body>
        <Card.Footer style={{ backgroundColor: "white", border: "none" }}>
          ...
          <Button onClick={handleShow} variant="custom">
            View Ingredients
          </Button>
        </Card.Footer>
      </Card>
            ...
            {ingredientLines.map((item) => (
              <div>
                <input
                  ...
                  name={item}
                  value={item}
                  checked={neededIngredients[item]}
