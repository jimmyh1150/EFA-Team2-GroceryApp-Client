...
const RecipeSearch = (props) => {
  ...
  const handleNavigateLastPage = () => {
    ...
    const totalPages =
      Math.floor(requestMetaRef.current.count / recipeLimit) - 1; // -1 so we dont start the request at the max value. we want the last page
        ...
        <div>
          {paginatedRecipes?.length > 0 && (
            <div>
              ...
              <Button
                onClick={nextClick}
                style={{ backgroundColor: "#0075A2", border: "none" }}
              >
                Next
              </Button>
          ...
          {visible && (
            <div
              style={{
                ...
                width: "400px",
                boxShadow: "2px 25px 25px #000000",
              }}
            >
              ...
              />
              <p>
                <i>Click the button to delete it from the list</i>
              </p>
              <GroceryList token={props.token} />
            </div>
          )}
          <button onClick={() => setVisible(!visible)} className="listBTN">
            <img src={ListIcon} className="ListIcon" alt="list" />
          </button>
        </div>
      </div>
      <br />
